...
#gör även en knapp för att rätta skiljefrågan

#gör olika filer och dela upp i olika classer. (kanske använd getter metoder för att klara detta)




#gör en mainfil där allt körs. Gör så att man kallar på Tips med ett antal inputparametrar t.ex width_cm

#gör grönt eller rött om man svarat rätt eller fel på frågan

#gör att man först bara displayar vad man gissat på skiljefrågan och denna rättas i slutet
...
#städa i koden och skriv kommentarer
...
class Tips:
    def __init__(self):
        ...
        self.y_const = 0
        self.x_const = 0
        self.picture_folderpath = r'C:\Users\jonat\OneDrive\chalmers\tipspromenad python\bildbank'
        self.answer_folderpath = r'C:\Users\jonat\OneDrive\chalmers\tipspromenad python\svar'
    ...
    def _get_picture_rbg(self,image_name, threshold, rgb_image, y, question):
        ...
        for x in self.X:
            ...
            i += 1
        if min_rgb > 220:
            print("oskäker på fråga", question, "med person", image_name)
...
# det svarta ifyllda är typ 135 men kan kolla på allt som är mindre än 200.
    ...
    def load_picture(self):
        os.chdir(self.picture_folderpath)
        images_names = []
        answers = []
    ...
    def get_one_correct_answers(self, question_number=None):
    ...
    def get_correct_txtanswers(self):
        os.chdir(self.answer_folderpath)
    ...
    def display_visual_score(self):
        ...
        for i in range(self.players):
            y = self.pixel_height-(self.pixel_height*(1/9*(self.increace_height*i+self.increace_height/2))+self.y_const)
            if i > self.leftside_players_amount-1:
                y = self.pixel_height-(self.pixel_height*(1/9*(self.increace_height*(i-self.leftside_players_amount)+self.increace_height/2))+self.y_const)
                x = self.pixel_width/2+self.pixel_width/16
            #self.leftside_players_amount-i-1
            visual_name = Text(Point(x, y), f"{self.sorted_by_skilje[i][0]}") #eftersom jag printar nedifrån vänster och upp
            visual_score = Text(Point(x+self.x_const, y), f"{self.sorted_by_skilje[i][1]}")
            visual_skilje = Text(Point(x+2*self.x_const, y), f"{self.sorted_by_skilje[i][2]}")
...
if __name__ == "__main__":
    ...
    #tips.get_one_correct_answers(1)
    
    tips.compare_skilje()
    tips.print_method()
    end = pc()
    print(f"Process took {round((end-start), 3)} secounds")
